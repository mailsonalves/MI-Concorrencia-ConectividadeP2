...
async def buy_ticket(
    ...
    if passagem.companhia_aerea == "VoeBr":
    ...
    elif passagem.companhia_aerea == "BrAirlines":
        return await attempt_buy_ticket_external(
            "http://localhost:8001/ticket/buy_ticket_no_login",
            passagem_data,
            str(user_id),
            db_session
        )
...
async def list(user_id: UUID, db_session: DatabaseSession, limit: int = 15, current_user=Depends(verify_login_current)):
    ...
    remote_server_urls = [
        f"http://127.0.0.1:8001/ticket/public-tickets?user_id={user_id}",
...
async def delete_ticket(
    ...
    if ticket.companhia_aerea != 'VoeBr':
        external_response = None
        try:
            if ticket.companhia_aerea == "BrAirlines":
                external_response = await delete_ticket_external("http://127.0.0.1:8001/ticket/delete_nologin", passagem_id)
            elif ticket.companhia_aerea == "BrasilPass":
